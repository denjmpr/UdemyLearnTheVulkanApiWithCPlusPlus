...
void VulkanRenderer::createGraphicsPipeline()
{
	...
	result = result = vkCreatePipelineLayout(mainDevice.logicalDevice, &secondPipelineLayoutCreateInfo, nullptr, &secondPipelineLayout);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create Pipeline Layout!");
	}
...
void VulkanRenderer::createDescriptorPool()
{
	...
	depthInputPoolSize.descriptorCount = static_cast<uint32_t>(colourBufferImageView.size());
...
int VulkanRenderer::createMeshModel(std::string modelFile)
{
	...
	std::vector<Mesh> modalMeshes = MeshModel::LoadNode(mainDevice.physicalDevice, mainDevice.logicalDevice, graphicsQueue, graphicsCommandPool,
		scene->mRootNode, scene, matToTex);

	MeshModel meshModel = MeshModel(modalMeshes);
...
stbi_uc* VulkanRenderer::loadTextureFile(std::string fileName, int* width, int* height, VkDeviceSize* imageSize)
{
	...
	if (!image)
	{
		throw std::runtime_error("Failed to create a Texture file! (" + fileName + ")");
	}
